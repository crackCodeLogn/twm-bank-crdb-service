...
class BankAccountDaoImplIntegrationTest {
    ...
    public void testAllOperations() {
        ...
        BankProto.BankAccount bankAccount1 = BankProto.BankAccount.newBuilder().setId(bankAccountId1).setBank(BankProto.Bank.newBuilder().setIFSC("TESTB-001")).setNumber("123456789012345").setName("Test Bank Account 001").setTransitNumber(999).setInstitutionNumber(9999).setBalance(91.19).setBankAccountType(BankProto.BankAccountType.CHQ).setOverdraftBalance(100).setInterestRate(0.05).setIsActive(true).build();
        bankAccountDao.getAllBankAccountsByMatchingIfsc("TESTB-001").ifPresent(accounts -> {
            accounts.getAccountsList().forEach(account -> bankAccountDao.deleteBankAccount(account.getId()));
        });
        ...
        BankProto.Bank testBank = BankProto.Bank.newBuilder().setIFSC("TESTB-001").setName("Test Bank 001").build();
        ...
        Optional<BankProto.BankAccount> optionalBankAccount1 = bankAccountDao.getBankAccount(bankAccountId1);
        ...
        BankProto.BankAccount bankAccount2 = BankProto.BankAccount.newBuilder().setId(bankAccountId2).setBank(BankProto.Bank.newBuilder().setIFSC("TESTB-001")).setNumber("1234567890").setName("Test Bank Account 002").setTransitNumber(998).setInstitutionNumber(9998).setBalance(5.23).setBankAccountType(BankProto.BankAccountType.HISA).setOverdraftBalance(100).setInterestRate(3.0).setIsActive(true).build();
        assertTrue(bankAccountDao.addBankAccount(bankAccount2));

        String bankAccountId3 = UUID.randomUUID().toString();
        BankProto.BankAccount bankAccount3 = BankProto.BankAccount.newBuilder().setId(bankAccountId3).setBank(BankProto.Bank.newBuilder().setIFSC("TESTB-001")).setNumber("12345678902324").setName("Sample 002").setTransitNumber(998).setInstitutionNumber(9998).setBalance(5.23).setBankAccountType(BankProto.BankAccountType.SAV).setOverdraftBalance(100).setInterestRate(1.5).setIsActive(true).build();
        ...
        Optional<BankProto.BankAccounts> bankAccountsList2 = bankAccountDao.getAllBankAccountsByMatchingName("Test ");
        assertTrue(bankAccountsList2.isPresent());
        assertEquals(2, bankAccountsList2.get().getAccountsCount());
        Set<String> bankAccs = bankAccountsList2.get().getAccountsList().stream().map(BankProto.BankAccount::getId).collect(Collectors.toSet());
        assertTrue(bankAccs.contains(bankAccountId1));
        assertTrue(bankAccs.contains(bankAccountId2));

        Optional<BankProto.BankAccounts> bankAccountsList3 = bankAccountDao.getAllBankAccountsByMatchingIfsc("TESTB");
