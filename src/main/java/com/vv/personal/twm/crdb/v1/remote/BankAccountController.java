...
public class BankAccountController {
    ...
    public String addBankAccounts(@RequestBody BankProto.BankAccounts bankAccounts) {
        ...
        try {
            ...
            log.info("Failed to add {} bank-accounts => {}", failedBankAccountIds.size(), failedBankAccountIds);
            return "FAILED";
        } catch (Exception e) {
            log.error("Failed to write all data correctly. ", e);
        }
    ...
    public BankProto.BankAccounts getBankAccounts(@RequestParam("field") String field, //NAME, IFSC, EMPTY - return
                                                  // all if EMPTY
        ...
        try {
            optionalBankAccounts = switch (field) {
    ...
    public boolean backUp(@RequestParam("destinationFolder") String destinationFolder, @RequestParam("delimiter") String delimiter) {
        ...
        boolean result = BankHelperUtil.writeToFile(bankAccountService.extractDataInDelimitedFormat(delimiter), destinationFolder, destinationFileName);
